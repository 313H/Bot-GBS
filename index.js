...
clientID.once("ready", async () => {
  ...
  try {
    ...
    let currentId =
      lastId === 0 ? parseInt(process.env.START_ID) : lastId + 1;

    for (const [id, member] of sortedMembers) {
      if (
        process.env.ASSIGN_ONLY_NEW === "true" &&
        /\b\d+\b/.test(member.displayName)
      )
        continue;
...
const logWebhook = process.env.ZAGL_LOG_WEBHOOK
  ? new WebhookClient({ url: process.env.ZAGL_LOG_WEBHOOK })
  : null;

clientTag.once("ready", () => {
  console.log(`✅ [ZAGL BOT] ${clientTag.user.tag} شغال`);
});
...
clientTag.on("messageCreate", async (message) => {
  ...
  if (message.content === "!zagl") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `أرسل رسالة لشخص آخر بطريقة أنيقة عبر الحمام الزاجل ✉️\n\nحالة البوت: **${isOnline}**`
      )
      .setColor("Blue");
...
clientTag.on(Events.InteractionCreate, async (interaction) => {
  if (interaction.isButton() && interaction.customId === "send_message") {
    const modal = new ModalBuilder()
      .setCustomId("zagl_modal")
      .setTitle("📨 إرسال رسالة زاجل");
    const userIdInput = new TextInputBuilder()
      .setCustomId("user_id")
      .setLabel("أدخل كوبي آيدي الشخص المستلم")
  ...
  if (interaction.isModalSubmit() && interaction.customId === "zagl_modal") {
    ...
    if (cooldown.has(sender.id)) {
      ...
      if (remaining > 0) {
        ...
        await interaction.reply({
          content: `⏳ يمكنك إرسال رسالة أخرى بعد ${minutes} دقيقة.`,
          ephemeral: true,
        });
        return;
      }
    ...
    if (
      config.BAD_WORDS.some((word) =>
        msgContent.toLowerCase().includes(word.toLowerCase())
      )
    ) {
      await interaction.reply({
        content: "🚫 رسالتك تحتوي على كلمات غير مسموح بها.",
        ephemeral: true,
      });
      return;
    }
    ...
    const anonButtons = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(
          `anon_yes_${userId}_${sender.id}_${Buffer.from(msgContent).toString("base64")}`
        )
        .setLabel("✅ إرسال كمجهول")
        .setStyle(ButtonStyle.Success),
      new ButtonBuilder()
        .setCustomId(
          `anon_no_${userId}_${sender.id}_${Buffer.from(msgContent).toString("base64")}`
        )
        .setLabel("❌ إرسال باسمك")
        .setStyle(ButtonStyle.Secondary)
    );
    await interaction.reply({
      content: "هل ترغب بإرسال الرسالة كمجهول الهوية؟",
  ...
  if (interaction.isButton() && interaction.customId.startsWith("anon_")) {
    const [_, mode, targetId, senderId, encodedMsg] =
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setTitle("📨 رسالة زاجل وصلت إليك 🕊️")
        .setDescription(msgContent)
        .setColor("Green")
        .setTimestamp();
      if (mode === "no")
        embed.setFooter({ text: `من ${sender.tag}`, iconURL: sender.displayAvatarURL() });
      ...
      if (logWebhook) {
        const logEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "👤 المرسل", value: `${sender.tag} (<@${sender.id}>)` },
            { name: "🎯 المستلم", value: `${target.tag} (<@${target.id}>)` },
            { name: "💬 الرسالة", value: msgContent.length > 1000 ? msgContent.slice(0, 1000) + "..." : msgContent },
            { name: "⚙️ نوع الإرسال", value: mode === "yes" ? "مجهول الهوية 🕵️" : "عادي 👤" },
      ...
      await interaction.update({
        content: "⚠️ حدث خطأ أثناء الإرسال. تأكد أن المستخدم يستقبل الرسائل الخاصة.",
        components: [],
      });
    }
  }
